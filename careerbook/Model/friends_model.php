...
class FriendsModel extends model {
    ...
    public function getFrndsReqData()
    {
	$user_id=17;
        $this->db->Fields(array('u.id','u.first_name','u.middle_name','u.last_name','u.gender','u.email_primary','f.status'));
        $this->db->From('users u');
    ...
    public function getAllUsersData($searchVal)
    {
    	$user_id=17;
    	$this->db->Fields(array('u.id','u.first_name','u.middle_name','u.last_name','u.gender','u.email_primary','f.status'));
    	$this->db->From('users u');
    	...
    	$this->db->From('users');
    	$this->db->Where(array("(status = 'A' AND (first_name like '%".$searchVal."%' OR middle_name like '%".$searchVal."%' OR last_name like '%".$searchVal."%' OR email_primary like '%".$searchVal."%')) AND id NOT IN (".$myStr.") "),true);
    	$this->db->Select();
